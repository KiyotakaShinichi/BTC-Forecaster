...
class FeatureAggregator:
    ...
    def aggregate(self, signals: list[EventSignal], forecast_origin: datetime) -> dict[str, float]:
        ...
        inflows = [
            s
            for s in recent72
            if s.event_type == EventType.WHALE_TRANSFER and s.transfer_context == TransferContext.EXCHANGE_INFLOW
        ]
        macro = [
            s
            for s in recent72
            if s.event_type in {EventType.MACRO_SHOCK, EventType.MONETARY_POLICY, EventType.LIQUIDITY_EVENT}
        ]
        return {
    ...
    def _weighted_mean(signals: list[EventSignal]) -> float:
        ...
        total = sum(weights)
        return sum(s.sentiment * w for s, w in zip(signals, weights, strict=False)) / total if total else 0.0
