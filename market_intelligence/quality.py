...
def evidence_confidence_multiplier(document: Document) -> float:
    ...
    score = (
        0.25 * float(metadata.official_source)
        + 0.20 * float(metadata.primary_source)
        + 0.10 * float(metadata.known_publisher)
        + 0.25 * metadata.timestamp_quality
...
def evaluate_quality(
    documents: list[Document], events: list[EventSignal], as_of: datetime | None = None
) -> QualityReport:
    ...
    for doc in documents:
        if doc.available_at.utcoffset() != timezone.utc.utcoffset(doc.available_at):
            issues.append(
                QualityIssue(
                    code="NON_UTC_TIMESTAMP", record_id=doc.document_id, detail="document availability not UTC"
                )
            )
        if as_of and doc.available_at > as_of:
            issues.append(
                QualityIssue(code="FUTURE_AVAILABILITY", record_id=doc.document_id, detail="document is future-known")
            )
    ...
    for event in events:
        ...
        if missing:
            issues.append(
                QualityIssue(code="ORPHANED_EVENT", record_id=event.event_id, detail=",".join(sorted(missing)))
            )
        if as_of and event.available_time > as_of:
            issues.append(
                QualityIssue(code="FUTURE_AVAILABILITY", record_id=event.event_id, detail="event is future-known")
            )
