...
from market_intelligence.configuration import (EntityType, ProviderCategory, ProviderConfig, ProviderRegistry,
    QueryPlanner, QuerySpec, WatchEntity, configuration_fingerprint)
...
from market_intelligence.models import (Direction, Document, EventSignal, EventType, ExtractionMethod,
    RetrievalProvenance, SignalCategory, SourceMetadata, SourceType, TransferContext)
from market_intelligence.operations import (BackfillManifest, HealthState, IntelligenceSnapshot,
    QuarantineRecord, Watermark, WatermarkStatus)
...
def doc(identifier="d1", provider="p1", query="btc", available=None, **changes):
    ...
    data = dict(document_id=identifier, url=f"https://example.com/{identifier}", publisher="Example",
        title="SEC regulation applies to Bitcoin", published_at=available - timedelta(minutes=5),
        retrieved_at=available, available_at=available, text_hash=Document.content_hash(identifier),
...
def event(identifier="e1", source_ids=("d1",), available=None, **changes):
    ...
    data = dict(event_id=identifier, event_time=available, available_time=available, source_ids=source_ids,
        category=SignalCategory.WEB_EVENT, event_type=EventType.REGULATION, direction=Direction.UNKNOWN,
        sentiment=0.0, btc_relevance=0.6, novelty=0.5, confidence=0.5, expected_horizon_hours=24,
        summary="event", extractor_version="fixture-v1", extraction_method=ExtractionMethod.FIXTURE)
    data.update(changes)
    return EventSignal(**data)


def query(identifier="q1", text="btc"):
    return QuerySpec(query_id=identifier, query=text, topic="Bitcoin", entities=("Bitcoin",),
        event_types=(EventType.REGULATION,), lookback_hours=24, priority=50, generated_at=NOW)


def config(identifier="p1", provider_type="fixture", timeout=1):
    return ProviderConfig(id=identifier, type=provider_type, priority=1, timeout=timeout,
        source_category=ProviderCategory.NEWS)
...
class ConfigurationTests(unittest.TestCase):
    ...
    def test_watchlist_and_query_planner_are_deterministic(self):
        watch = [WatchEntity(canonical_name="Jerome Powell", aliases=("Powell",), entity_type=EntityType.CENTRAL_BANK,
            topics=("Federal Reserve", "interest rates"), expected_event_types=(EventType.MONETARY_POLICY,),
            importance_prior=0.7, measured_impact=None)]
...
class RetrievalTests(unittest.TestCase):
    def test_retry_backoff_hook_and_eventual_success(self):
        ...
        result = MultiProviderRetriever({"p1": provider}, {"p1": config()}, max_retries=2,
            sleep=sleeps.append, jitter=lambda: 0.0).retrieve([query()])
    ...
    def test_provider_failure_is_partial_success(self):
        class Failed(FixtureSearchProvider):
            def search(self, *args): raise RuntimeError("down")
    ...
    def test_cross_provider_dedup_preserves_provenance(self):
        ...
        b = doc("vendor-id", provider="b", url="https://example.com/story", text_hash="same",
            retrieved_at=NOW - timedelta(minutes=30), available_at=NOW - timedelta(minutes=30))
    ...
    def test_source_metadata_is_operational_not_political(self):
        metadata = SourceMetadata(source_type=SourceType.PRIMARY_OFFICIAL, official_source=True,
            primary_source=True, known_publisher=True, timestamp_quality=1, content_completeness=.8)
...
class ExtractionTests(unittest.TestCase):
    def test_llm_rejects_nan_and_future_time(self):
        raw_nan = event(extractor_version="v2").model_dump(mode="json"); raw_nan["sentiment"] = float("nan")
    ...
    def test_rule_whale_unknown_context(self):
        ...
        self.assertEqual((signal.event_type, signal.transfer_context, signal.direction),
            (EventType.WHALE_TRANSFER, TransferContext.UNKNOWN, Direction.UNKNOWN))

    def test_social_and_whale_are_interfaces(self):
        self.assertTrue(getattr(SocialStatementProvider, "__abstractmethods__"))
        self.assertTrue(getattr(WhaleDataProvider, "__abstractmethods__"))
...
class PersistenceReplayTests(unittest.TestCase):
    def test_watermark_advances_only_after_atomic_storage(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            watermark = Watermark(provider_id="p1", query_id="q1", last_successful_available_time=NOW,
                last_retrieval_time=NOW, last_document_id="d1", status=WatermarkStatus.SUCCESS)
            original = store.put_signals
            store.put_signals = lambda _: (_ for _ in ()).throw(RuntimeError("disk"))
            with self.assertRaises(RuntimeError): store.persist_cycle([doc()], [event()], [watermark])
            self.assertEqual(store.documents_as_of(NOW), [])
            self.assertIsNone(store.get_watermark("p1", "q1"))
            store.put_signals = original; store.persist_cycle([doc()], [event()], [watermark])
    ...
    def test_failed_watermark_does_not_advance(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store.persist_cycle([], [], [failed]); self.assertIsNone(store.get_watermark("p1", "q1")); store.close()

    def test_replay_and_snapshot_exclude_future_membership(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = IntelligenceStore(Path(tmp) / "x.db")
            past, future = doc(), doc("future", available=NOW + timedelta(seconds=1), retrieved_at=NOW + timedelta(seconds=1))
            past_event, future_event = event(), event("future-event", ("future",), available=NOW + timedelta(seconds=1))
            store.put_documents([past, future]); store.put_signals([past_event, future_event])
            snap = ReplayService(store).replay(NOW, {"p1": "v1"}, "cfg")
            self.assertEqual((snap.document_ids, snap.event_ids), (("d1",), ("e1",)))
            self.assertEqual(snap.feature_contract_version, FEATURE_CONTRACT_VERSION); store.close()

    def test_snapshot_fingerprint_is_deterministic_and_membership_sensitive(self):
        kwargs = dict(forecast_origin=NOW, document_ids=["d1"], event_ids=["e1"], features={"x": 1.0},
            provider_versions={"p": "1"}, extractor_versions=["x"], configuration_fingerprint="c",
            source_hashes=["h"], feature_contract_version="v1")
        first = IntelligenceSnapshot.create(**kwargs); second = IntelligenceSnapshot.create(**kwargs)
        self.assertEqual(first.snapshot_id, second.snapshot_id)
        self.assertNotEqual(first.snapshot_id, IntelligenceSnapshot.create(**{**kwargs, "event_ids": ["e2"]}).snapshot_id)
    ...
    def test_backfill_is_bounded_and_resumable(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            resumed = BackfillRunner(lambda a, b: calls.append((a, b))).run(
                first.model_copy(update={"max_windows": 5}), path)
...
class OperationsTests(unittest.TestCase):
    ...
    def test_quarantine_hashes_raw_without_storing_it(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            store = IntelligenceStore(Path(tmp) / "x.db"); store.put_quarantine([record])
            self.assertEqual(store.quarantine_count(), 1); store.close()

    def test_structured_logger_redacts_secret_fields(self):
        stream = io.StringIO(); handler = logging.StreamHandler(stream); logger = logging.Logger("test"); logger.addHandler(handler)
        StructuredRunLogger(logger).emit("attempt", provider="p", api_key="secret", authorization="token")
        self.assertIn('"provider": "p"', stream.getvalue()); self.assertNotIn("secret", stream.getvalue())
    ...
    def test_cycle_partial_failure_missingness_health_and_manifest(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            store = IntelligenceStore(Path(tmp) / "x.db"); manifest = Path(tmp) / "manifest.json"
            report = run_intelligence_cycle([query()], MultiProviderRetriever(providers, configs, max_retries=0),
                RuleBasedExtractor(), store, configs, NOW, manifest, now=lambda: NOW, source_sha="abc")
            ...
            self.assertTrue(manifest.exists()); self.assertEqual(json.loads(manifest.read_text())["software_source_sha"], "abc")
            store.close()

    def test_extractor_failure_keeps_document_and_quarantines(self):
        class BadExtractor(FixtureExtractor):
            version = "bad"
            def extract(self, documents): raise ExtractionError("bad json")
        with tempfile.TemporaryDirectory() as tmp:
            store = IntelligenceStore(Path(tmp) / "x.db")
            report = run_intelligence_cycle([query()], MultiProviderRetriever({"p1": FixtureSearchProvider([doc()])}, {"p1": config()}, max_retries=0),
                BadExtractor([]), store, {}, NOW, now=lambda: NOW)
            self.assertEqual((len(store.documents_as_of(NOW)), len(report.events), store.quarantine_count()), (1, 0, 1)); store.close()
...
class CliAndEvaluationTests(unittest.TestCase):
    def test_cli_parser_has_all_commands(self):
        for command in ("collect", "extract", "aggregate", "replay", "backfill", "health", "quality"):
            with self.subTest(command=command):
                args = [command]
                if command in {"extract", "aggregate", "replay", "quality"}: args += ["--origin", NOW.isoformat()]
                elif command == "collect": args += ["--config", "x", "--origin", NOW.isoformat(), "--manifest", "m"]
                elif command == "backfill": args += ["--from", NOW.isoformat(), "--to", (NOW + timedelta(hours=1)).isoformat(), "--config", "x"]
    ...
    def test_gold_evaluation_and_comparison(self):
        documents = [doc()]
        labels = [GoldLabel(case_id="reg", document_ids=("d1",), expected_event_type=EventType.REGULATION,
            expected_entity="SEC", relevance_min=.5, relevance_max=.7)]
